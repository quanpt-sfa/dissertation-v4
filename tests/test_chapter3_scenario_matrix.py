...
CONFIG_PATH = (
    ROOT / "config" / "execution" / "simulation.yaml"
)
...
def test_chapter3_scenario_matrix_is_locked_and_complete_for_supported_dgp() -> None:
    ...
    assert (
        scenario_blocks
        == contract["required_scenario_blocks"]
    )
    assert (
        set(contract["baseline_scenario_ids"])
        <= set(scenario_ids)
    )
    ...
    assert all(
        float(item["shift_strength"]) == 0.0
        for item in scenarios
    )

    implemented = set(
        contract["implemented_engine_blocks"]
    )
    assert implemented == {
    ...
    deferred = {
        item["block"]
        for item in contract["deferred_engine_blocks"]
    }
    assert deferred == {
    ...
    assert (
        simulation["l3_correct_and_misspecified_checked"]
        is True
    )
...
def test_chapter3_measurement_profile_holds_learner_fixed() -> None:
    ...
    imbalance = build_imbalance_treatment_contract(
        simulation
    )
    ...
    predictive = [
        item
        for item in active
        if item["method_family"] == "predictive"
    ]
    standalone = [
        item
        for item in active
        if item["method_family"]
        == "standalone_estimator"
    ]
