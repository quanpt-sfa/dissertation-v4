...
def validate_decisions(registry: dict[str, object]) -> list[dict[str, object]]:
    ...
    for decision_id in sorted(decisions):
        ...
        if (
            ...
            raise DecisionTraceabilityError(
                f"decision={decision_id}: completed non-placeholder statement required"
            )
        ...
        if not isinstance(item.get("chapter_reference"), str) or not item["chapter_reference"]:
            raise DecisionTraceabilityError(f"decision={decision_id}: chapter_reference required")
