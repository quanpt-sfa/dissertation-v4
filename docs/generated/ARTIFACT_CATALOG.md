GENERATED FILE — DO NOT EDIT
Source: config/pipeline.yaml

# Artifact catalog
| ID | Producer | Schema | Path |
|---|---|---|---|
| ablation_results | P13 | json_array | `P13/ablations.json` |
| access_matrix | P00 | json_object | `P00/access_matrix.json` |
| artifact_catalog | P00 | json_object | `P00/artifact_catalog.json` |
| availability_registry | P03 | json_array | `P03/availability_registry.json` |
| bootstrap_batches | P12 | json_array | `P12/bootstrap/{outer_fold}.json` |
| calibration_outputs | P12 | json_object | `P12/calibration/{outer_fold}.json` |
| capability_seed | P00 | json_object | `P00/capability_seed.json` |
| channel_measurement_selection | P10 | json_object | `P10/channel_selection/{outer_fold}.json` |
| channel_time_split_registry | P09 | json_array | `P09/channel_splits.json` |
| decision_traceability | P00 | json_array | `P00/decision_traceability.json` |
| domain_transfer_outputs | P13 | json_object | `P13/domain_transfer.json` |
| duplicate_map | P02 | json_array | `P02/duplicate_map.json` |
| environment_expectation | P00 | json_object | `P00/environment_expectation.json` |
| evaluation_metrics | P12 | json_object | `P12/metrics/{outer_fold}.json` |
| evidence_ledger | P03 | firm_year_frame | `P03/evidence_ledger.parquet` |
| feature_panel | P07 | firm_year_frame | `P07/feature_panel.parquet` |
| feature_registry | P07 | json_array | `P07/feature_registry.json` |
| final_report | P17 | markdown_document | `P17/final_report.md` |
| final_result_ledger | P17 | json_array | `P17/final_ledger.json` |
| firm_master | P02 | firm_year_frame | `P02/firm_master.parquet` |
| firm_year_panel | P02 | firm_year_frame | `P02/firm_year_panel.parquet` |
| fold_aware_weights | P09 | firm_year_frame | `P09/weights/{fold_id}.parquet` |
| gate2_verdict | P14 | receipt_schema | `P14/gate2.json` |
| gate3_verdict | P16 | receipt_schema | `P16/gate3.json` |
| hierarchical_pi_sensitivity | P13 | json_object | `P13/hierarchical_pi.json` |
| job_manifest | P00 | json_object | `P00/job_manifest.json` |
| known_case_results | P15 | json_array | `P15/known_cases.json` |
| known_cases_seal | P00 | receipt_schema | `P00/known_cases_seal.json` |
| l0_l1_inputs | P05 | firm_year_frame | `P05/l0_l1_inputs.parquet` |
| l3_pilot_capability | P05 | json_object | `P05/l3_pilot_capability.json` |
| lag_decomposition | P03 | json_object | `P03/lag_decomposition.json` |
| maturity_audit | P04 | json_object | `P04/maturity_audit.json` |
| mcse_report | P08 | json_object | `P08/mcse_report.json` |
| measurement_candidate_results | P10 | json_array | `P10/candidates/{outer_fold}.json` |
| measurement_selection_registry | P10 | json_object | `P10/selection/{outer_fold}.json` |
| model_artifacts | P11 | json_object | `P11/models/{outer_fold}.json` |
| model_freeze_receipt | P11 | receipt_schema | `P11/freeze/{outer_fold}.json` |
| observability_registry | P06 | json_object | `P06/observability_registry.json` |
| outer_open_receipt | P12 | receipt_schema | `P12/outer_open/{outer_fold}.json` |
| p00_audit_report | P00 | markdown_document | `P00/P00_AUDIT_REPORT.md` |
| protocol_hash | P00 | text_value | `P00/protocol_hash.txt` |
| raw_audit | P01 | json_object | `P01/raw_audit/{source_id}.json` |
| raw_outer_predictions | P12 | firm_year_frame | `P12/predictions/{outer_fold}.parquet` |
| registry_lock | P00 | json_object | `P00/registry.lock.json` |
| risk_sets | P04 | firm_year_frame | `P04/risk_sets.parquet` |
| schema_catalog | P00 | json_object | `P00/schema_catalog.json` |
| simulation_batches | P08 | json_array | `P08/batches/{scenario_id}.json` |
| simulation_scenario_registry | P08 | json_array | `P08/scenario_registry.json` |
| source_channel_matrices | P05 | json_object | `P05/source_channel_matrices.json` |
| source_config_manifest | P00 | json_object | `P00/source_config_manifest.json` |
| step_catalog | P00 | json_object | `P00/step_catalog.json` |
| success_receipt | P00 | receipt_schema | `P00/_SUCCESS.json` |
| temporal_split_registry | P09 | json_array | `P09/temporal_splits.json` |
| threshold_interaction_results | P16 | json_object | `P16/threshold_interactions.json` |
| utility_scenarios | P12 | json_array | `P12/utility/{outer_fold}.json` |
